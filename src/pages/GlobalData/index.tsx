...
import {listData, lineOneData, lineTwoData, pieData} from '../../../public/fakeData'
import {useSelector} from '@@/plugin-dva/exports'

const {Content} = Layout

const GlobalDataPage: FC = () => {
  const {listData, lineOneData, lineTwoData, pieData} = useSelector<ConnectState, DataModelState>(state => state.data)
      ...
      <Layout className={styles.layout}>
        ...
        <Content>
          ...
          <div className={styles.content_two}>
            <div className={styles.content_title}>Pekka分布式云渲染全球节点实时动态数据</div>
            <div className={styles.content_subtitle}>用全球分布式去中心化区块链技术改变云渲染产业，让世界更高效</div>
          ...
          <div className={styles.content_three}>
            ...
            <div className={styles.three_data_table_area}>
              <div className={styles.table_button_area}>
                <Button type='default' size='small' onClick={() => handleChangeOneType('day')}>24时</Button>
                <Button type='default' size='small' onClick={() => handleChangeOneType('week')}>7天</Button>
                <Button type='default' size='small' onClick={() => handleChangeOneType('month')}>30天</Button>
                <Button type='default' size='small' onClick={() => handleChangeOneType('year')}>1年</Button>
              </div>
          ...
          <div className={styles.content_four}>
            ...
            <div className={styles.four_data_table_area}>
              <div className={styles.table_button_area}>
                <Button type='default' size='small' onClick={() => handleChangeTwoType('day')}>24时</Button>
                <Button type='default' size='small' onClick={() => handleChangeTwoType('week')}>7天</Button>
                <Button type='default' size='small' onClick={() => handleChangeTwoType('month')}>30天</Button>
                <Button type='default' size='small' onClick={() => handleChangeTwoType('year')}>1年</Button>
              </div>
          ...
          <div className={styles.content_six}>
            <div className={styles.content_title}>共享全球算力，开启云渲染服务</div>
            <div
              className={styles.content_subtitle}>Pekka节点算力来自全球各地，实现海量节点拓展，提供全球性急速渲染服务，用户遍及美洲、亚洲、欧洲、中东等区域，为全球用户带来极致非凡的云渲染解决方案服务。
            </div>
